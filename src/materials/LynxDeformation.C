...
validParams<LynxDeformation>()
{
  ...
  params.addParam<MooseEnum>("strain_model",
                             LynxDeformation::strainModel() = "small",
...
LynxDeformation::LynxDeformation(const InputParameters & parameters)
  : LynxMaterialBase(parameters),
    ...
    _friction_angle_0((_has_plasticity && isParamValid("friction_angle"))
                          ? getLynxParam<Real>("friction_angle")
                          : std::vector<Real>(_n_composition, 0.0)),
    _cohesion_0((_has_plasticity && isParamValid("cohesion"))
                    ? getLynxParam<Real>("cohesion")
                    : std::vector<Real>(_n_composition, 0.0)),
    _friction_angle_res((_has_plasticity && isParamValid("friction_angle_residual"))
                            ? getLynxParam<Real>("friction_angle_residual")
                            : _friction_angle_0),
    _cohesion_res((_has_plasticity && isParamValid("cohesion_residual"))
                      ? getLynxParam<Real>("cohesion_residual")
                      : _cohesion_0),
    _dilation_angle((_has_plasticity && isParamValid("dilation_angle"))
                        ? getLynxParam<Real>("dilation_angle")
                        : std::vector<Real>(_n_composition, 0.0)),
    _intnl_0((_has_plasticity && isParamValid("internal_0"))
                 ? getLynxParam<Real>("internal_0")
                 : std::vector<Real>(_n_composition, 0.0)),
    _intnl_lim((_has_plasticity && isParamValid("internal_limit"))
                   ? getLynxParam<Real>("internal_limit")
                   : std::vector<Real>(_n_composition, 1.0)),
    _one_on_plastic_eta((_has_plasticity && isParamValid("plastic_viscosity"))
                            ? getLynxParam<Real>("plastic_viscosity")
                            : std::vector<Real>(_n_composition, 0.0)),
...
MooseEnum
LynxDeformation::strainModel()
{
  return MooseEnum("small=0 finite=1");
}

MooseEnum
LynxDeformation::viscousUpdate()
{
...
void
LynxDeformation::computeProperties()
{
...
void
LynxDeformation::computeQpProperties()
{
...
void
LynxDeformation::computeStrainIncrement()
{
...
void
LynxDeformation::calculateSmallStrain(const RankTwoTensor & grad_tensor,
...
void
LynxDeformation::calculateFiniteStrain(const RankTwoTensor & grad_tensor,
                                       const RankTwoTensor & grad_tensor_old)
{
...
void
LynxDeformation::computeQpDeformation()
{
...
void
LynxDeformation::volumetricDeformation(Real & pressure)
{
  if (_coupled_pdyn)
...
void
LynxDeformation::deviatoricDeformation(const Real & pressure, RankTwoTensor & stress_dev)
{
...
void
LynxDeformation::tangentOperator()
{
  RankTwoTensor identity_two(RankTwoTensor::initIdentity);
  RankFourTensor volumetric_four = identity_two.outerProduct(identity_two);
  RankFourTensor identity_four(RankFourTensor::initIdentitySymmetricFour);
  RankFourTensor deviatoric_four = identity_four - volumetric_four / 3.0;
  ...
  // Elastic operator
  _tangent_modulus[_qp] = (_G[_qp] != 0.0) ? 2.0 * _G[_qp] * deviatoric_four
                                           : 2.0 * _eta_eff[_qp] * deviatoric_four / _dt;
                                           
  _tangent_modulus[_qp] += _K[_qp] * volumetric_four;

  // Viscous correction
  _tangent_modulus[_qp] -= (_eta_eff[_qp] != 0.0 && _G[_qp] != 0.0)
                               ? 2.0 * _G[_qp] *
                                     ((1.0 - _stress_corr_v) * deviatoric_four +
  ...
  {
    RankFourTensor plastic_operator = (1.0 - _stress_corr_p) * deviatoric_four +
                                       1.5 * (_stress_corr_p - _dq_dq_tr_p) * flow_direction_dyad;
    plastic_operator +=
        0.5 * _K[_qp] / _G[_qp] * _dq_dp_tr_p * flow_direction.outerProduct(identity_two);
    plastic_operator += (1.0 - _dp_dp_tr_p) * volumetric_four / 3.0;
    plastic_operator +=
        3.0 * _G[_qp] / _K[_qp] * _dp_dq_tr_p * identity_two.outerProduct(flow_direction) / 3.0;
    _tangent_modulus[_qp] = _tangent_modulus[_qp] * (identity_four - plastic_operator);
  }
...
void
LynxDeformation::finiteTangentOperator()
{
...
Real
LynxDeformation::computeStokeEffectiveViscosity(const Real & pressure)
{
...
RankTwoTensor
LynxDeformation::computeStokeEffectiveViscosityDerivative()
{
...
Real
LynxDeformation::computeEffectiveViscosity(const Real & pressure, const Real & eqv_stress)
{
  computeCreepProperties(pressure);
...
Real
LynxDeformation::computeCreepRate(const Real A, const Real n, const Real eqv_stress)
{
...
Real
LynxDeformation::viscousIncrement(const Real & pressure, const Real & eqv_stress)
{
...
LynxDeformation::plasticIncrement(const Real & pressure, const Real & eqv_stress)
{
  ...
  if (_plastic_yield_function[_qp] > 0.0)
  {
    ...
    // Update yield
    _plastic_yield_function[_qp] -= (3.0 * _G[_qp] + _plasticity->_H) * eqv_p_strain_incr;

    _stress_corr_p =
        (eqv_stress != 0.0) ? (eqv_stress - 3.0 * _G[_qp] * eqv_p_strain_incr) / eqv_stress : 1.0;
    _dp_dp_tr_p = 1.0 - _plasticity->_alpha * _plasticity->_beta * _K[_qp] * vp_correction / (3.0 * _G[_qp] + _plasticity->_H);
    _dp_dq_tr_p = _plasticity->_beta * _K[_qp] * vp_correction / (3.0 * _G[_qp] + _plasticity->_H);
    _dq_dp_tr_p = _plasticity->_alpha * 3.0 * _G[_qp] * vp_correction / (3.0 * _G[_qp] + _plasticity->_H);
...
RankTwoTensor
LynxDeformation::spinRotation(const RankTwoTensor & tensor)
{
  return tensor + _spin_tensor[_qp] * tensor.deviatoric() - tensor.deviatoric() * _spin_tensor[_qp];
}

void
LynxDeformation::updateSpinTangentModulus()
{
...
void
LynxDeformation::computeCreepProperties(const Real & pressure)
{
...
void
LynxDeformation::updateCreepParameters()
{
  if (_has_diffusion_creep)
...
Real
LynxDeformation::rootBrent(iterative_viscous & viscous_model, const Real x1, const Real x2)
{
...
Real
LynxDeformation::rootNewtonSafe(iterative_viscous & viscous_model, const Real x1, const Real x2)
{
  // Machine floating-point precision
  const Real eps = std::numeric_limits<Real>::epsilon();
  // Declare some stuff here
...
void
LynxDeformation::computeQpThermalSources()
{
