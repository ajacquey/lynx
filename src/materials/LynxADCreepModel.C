                                                  ...
                                                  const ADReal & G)
{
  ...
  if ((_eta_eff[_qp] < this->averageProperty(_eta_min)) ||
      (_eta_eff[_qp] > this->averageProperty(_eta_max)))
    _eta_eff[_qp] = std::min(std::max(_eta_eff[_qp], this->averageProperty(_eta_min)),
  ...
  template <ComputeStage compute_stage>
  ADReal LynxADCreepModel<compute_stage>::computeQpEffectiveViscosity(const ADReal & pressure)
  {
    // Map creep parameters
    initCreepParameters(pressure);
    ...
    if (_t_step <= 1 && strain_rate_II == 0.0 && _has_initial_viscosity)
      return std::min(
          std::max(this->averageProperty(_initial_viscosity), this->averageProperty(_eta_min)),
          this->averageProperty(_eta_max));
    ...
    return std::min(std::max(eta, this->averageProperty(_eta_min)),
                    this->averageProperty(_eta_max));
  }

  template <ComputeStage compute_stage>
  ADReal LynxADCreepModel<compute_stage>::computeQpOneOnDiffViscosity(const ADReal A)
  {
    return 2.0 * A;
  }

  template <ComputeStage compute_stage>
  ADReal LynxADCreepModel<compute_stage>::computeQpOneOnDislViscosity(
  ...
  template <ComputeStage compute_stage>
  void LynxADCreepModel<compute_stage>::initCreepParameters(const ADReal & pressure)
  {
  ...
  template <ComputeStage compute_stage>
  ADReal LynxADCreepModel<compute_stage>::iterativeResidual(const ADReal & x)
  {
    return 2.0 * _G * (_A_diff * x + _A_disl * std::pow(x, _n_disl)) * _dt - (_tau_II_tr - x);
  }

  template <ComputeStage compute_stage>
  ADReal LynxADCreepModel<compute_stage>::iterativeResidualDerivative(const ADReal & x)
  {
    return 2.0 * _G * (_A_diff + _n_disl * _A_disl * std::pow(x, _n_disl - 1.0)) * _dt + 1.0;
  }

  template <ComputeStage compute_stage>
  ADReal LynxADCreepModel<compute_stage>::newtonRoot()
  {
  ...
  template <ComputeStage compute_stage>
  ADReal LynxADCreepModel<compute_stage>::brentRoot(const ADReal & x1, const ADReal x2)
  {
  ...
  // Recipes. The Art of Scientific Computing, 3rd Edition, 2007, (C++ code)
  template <ComputeStage compute_stage>
  ADReal LynxADCreepModel<compute_stage>::safeNewtonRoot(const ADReal & x1, const ADReal x2)
  {
    // Machine floating-point precision
    const Real eps = std::numeric_limits<Real>::epsilon();
    // Declare some stuff here
